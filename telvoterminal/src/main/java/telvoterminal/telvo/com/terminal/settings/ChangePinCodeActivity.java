...
public class ChangePinCodeActivity extends BaseActivity implements View.OnClickListener, IServiceResultListener {

    private OtpView oldPinCodeOtpView, newPinCodeOtpView, confirmPinCodeOtpView;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setToolbar(R.id.toolbar_change_pin_code, getStringResources(R.string.change_pin));
    ...
    private void initializeViews() {
        oldPinCodeOtpView = findViewById(R.id.oldPinCodeOtpView);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.submitButton:
                ...
                if (Validation.isTextEmpty(oldPinCodeOtpView.getOTP())) {
                    done = false;
                    customAlertDialog.showDialog(getStringResources(R.string.enter_old_pin_code));
                } else if (!oldPinCodeOtpView.hasValidOTP()) {
                    done = false;
                    customAlertDialog.showDialog(getStringResources(R.string.invalid_old_pin_size));
                } else if (Validation.isTextEmpty(newPinCodeOtpView.getOTP())) {
                    done = false;
                ...
                }else if (!user.getPinCode().equals(oldPinCodeOtpView.getOTP())) {
                    done = false;
                    customAlertDialog.showDialog(getStringResources(R.string.old_pin_mismatch_msg));
                }else if (!((newPinCodeOtpView.getOTP()).equals(confirmPinCodeOtpView.getOTP()))) {
                    done = false;
